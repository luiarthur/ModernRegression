...
observed random variables follow a multibariate normal distribution.
...
\subsubsection*{Full Gaussian Process Model:}  
  ...
  %Using the results the conditional expectation and contitional variance (see
...
\subsubsection*{Parameter Estimats:}
% tau2, s2, phi
